...
class BinanceWebSocketClient(MarketDataClient):
    def __init__(self, symbol: str, interval: str, candlestick_service):
        self.symbol = symbol.lower()
        self.interval = interval
        self.url = f"wss://stream.binance.com:9443/ws/{self.symbol}@kline_{self.interval}"
        self.ssl_context = ssl.create_default_context(cafile=certifi.where())
    
    async def fetch_candlestick_data(self) -> Candle:
        async with websockets.connect(self.url, ssl=self.ssl_context) as ws:
            async for message in ws:
                ...
                candle = Candle(timestamp=kline[0], open=float(kline[1]), high=float(kline[2]),
...
class BinanceRestClient(MarketDataClient):
    ...
    async def fetch_candlestick_data(self, symbol: str, interval: str) -> List[Candle]:
        url = f"{self.BASE_URL}?symbol={symbol.upper()}&interval={interval}&limit=500"
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as response:
                data = await response.json()
                candles = [
                    Candle(timestamp=c[0], open=float(c[1]), high=float(c[2]),
